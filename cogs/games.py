    ...
    async def handleTurn(self, i: discord.Interaction):
        ...
        for way in self.waysToWin:
            ...
            if all(self.buttons[coord[0]][coord[1]].emoji == emoji for coord in way):
                end = True if self.buttons[way[0][0]][way[0][1]].clicked else False
        if end:
        ...
        if end:
            await i.followup.send(f"{self.client.get_user(self.playerPrev).name} hat das Spiel gewonnen!")
